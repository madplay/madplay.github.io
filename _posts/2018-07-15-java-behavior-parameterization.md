...
<pre class="line-numbers"><code class="language-java" data-start="1">import org.apache.commons.lang3.StringUtils;
...
<pre class="line-numbers"><code class="language-java" data-start="1">interface BookPredicate {
...
Java 8 버전부터는 ```람다 표현식(Lambda Expression)```을 사용할 수 있습니다.
코드가 간결하게 바뀝니다. 직접 바꿔봅시다. 
 ...
 * 이렇게 바뀝니다.
 */
List&lt;Book&gt; redBooks = filterBooks(bookList, book -&gt; StringUtils.equals(book.getColor(), "red"));
</code></pre>
 ...
 */
bookList.stream().filter(book -> book.getPage() > 400).collect(Collectors.toList());
</code></pre>

메서드 구현도 없이 몇 라인의 코드로 끝낼 수 있습니다.
```걸러내고(filter) 모은다(collect) 리스트로(toList)``` 어떻게 보면 읽기도 편한 것 같기도 합니다. 