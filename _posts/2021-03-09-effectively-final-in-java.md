...
description: 자바에서 final로 선언되지 않았지만 값이 재할당되지 않는 "effectively final" 이란 무엇일까?
category: Java
...
익명 클래스 또는 람다식에서는 참조하는 외부 지역 변수가 `final`로 선언됐거나 선언된 후 값이 변경되지 않는 **effectively final**인 경우에만 접근 가능하다.
...
# effectively final
그렇다면 **effectively final**은 정확히 어떤 경우를 말하는 것일까? 자바 언어 스펙에서는 다음과 같은 조건을 만족하는
지역 변수(local variables)로 정의한다.
...
그렇다면 람다 내부에서 외부 지역 변수의 값을 변경하려고 하는 경우 컴파일 오류가 발생하는 이유는 무엇일까?
...
다른 예제를 살펴보자. 이번 예제 코드는 컴파일 오류가 발생한다. 하지만 정상적으로 실행된다고 가정해보자.
...
설명을 위해 지역 변수를 제어하는 스레드를 A, 람다식을 실행하는 스레드를 B라고 가정해보자.

앞서 설명한 것처럼 람다에서는 외부 지역 변수를 사용하는 경우 캡처링을 한다. 그렇기 때문에 스레드 B에서는 람다식 내부에서
`doLoop` 지역 변수의 값을 참조하기 위해 이를 복사한다. 

문제는 여기서 발생할 수 있다. 복사되는 값인 외부 지역 변수가 변경 가능하게 되는 경우 복사된 값이 최신 값임을 보장할 수 없다.
이는 변수의 가시성(visibility)과도 연관이 있는데, 스택 영역은 스레드마다 생성되기 때문에 스레드 A, B가 갖는 스택 영역은 각자 고유하다.
따라서 다른 스레드의 스택에 있는 값의 변경사항을 확인할 수가 없다.
...
# 정리하면
람다 내부에서 외부에 선언된 지역 변수를 참조하는 경우 `final` 또는 **effectively final**이어야 한다.
이러한 이유는 지역 변수가 메모리 영역중 스택(Stack) 영역에 할당되는 것과 관련이 있다.

스택 영역은 스레드 별로 고유하기 때문에 지역 변수가 할당된 스레드가 종료되면 지역 변수를 참조하지 못하게 된다.
