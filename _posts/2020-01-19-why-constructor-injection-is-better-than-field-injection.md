...
대해서 알아보자. 등록된 빈을 사용하기 위해서 DI(Dependency Injection) 하는 방법은 3가지가 있다. 우선 어떤 종류가 있는지 살펴보자.
...
먼저 **수정자 주입(Setter Injection)을 살펴보면** 우선 주입(inject) 하려는 빈의 생성자를 호출하여 빈을 찾거나 빈 팩터리에 등록한다.
...
마지막으로 **생성자 주입(Constructor Injection)이다.** 생성자에서 주입하는 경우에는 생성자로 객체를 생성하는 시점에 필요한 빈들을 주입한다.
조금 더 자세히 살펴보면, 먼저 생성자의 인자에 사용되는 빈을 찾거나 빈 팩터리에서 만든다. 그 후에 찾은 인자 빈으로 주입하려는 빈의 생성자를 호출한다.
그렇기 때문에 참조하는 객체가 생성되지 않았음에도 참조하고 있기 때문에 발생한다.
...
아쉽게도 필드 주입과 수정자 주입은 final로 선언할 수 없다. 따라서 초기화 후에 객체가 변경될 수 있지만 생성자 주입의 경우 필드를 final로 선언할 수 있다.
