...
자바에서는 2가지의 객체 소멸자를 제공합니다. 바로 최상위 Object 클래스에 포함된 ```finalize``` 메서드와
...
사용을 권장하지는 않습니다. 그 이유를 하나씩 살펴봅시다.
...
**실행조차 안 될 수 있습니다.** 즉시 실행이 안되는 것을 감안한다고 하더라도 프로그램이 비정상 종료된다는 등의 이유로
실행조차 안 될 수 있습니다. 그렇기 때문에 특정 시점 또는 반드시 실행되어야 한다는 것을 기대하고 사용해서는 안 됩니다.
...
**실행이 느립니다.** ```AutoCloseable```을 구현(implements)한 객체를 만들고 ```try-catch-resource```로 자원을 반납하는데
...
사용한다고 합니다. 개인적으로 ```finalize```를 직접 호출하는 것과 ```close```를 안 하는 것 중 굳이 고르자면 어느 것이 조금 더
나은 방법일지 궁금하기도 합니다.
