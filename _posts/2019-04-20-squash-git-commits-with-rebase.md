...
subtitle: 여러 개의 커밋을 하나로 합치고 싶다. git rebase를 이용하여 커밋 로그를 묶어보자.
category: Knowledge
comments: true
---
...
그래도 커밋을 나누는 것이 **Pull Request**와 같이 리뷰를 진행할 때 보기 편하기는 한데,
오류를 고치거나 수정 사항을 반영하기 위해 커밋을 여러번하게 되는 경우가 있습니다.  
...
이후 ```:wq``` 명령어로 저장하여 편집화면을 종료하면 됩니다. 화면을 빠져나오자마자 아래와 같은 모습을 확인할 수 있습니다.
...
실제로는 조금 다르고 더 멋있게(?) 커밋 메시지를 적으시면 됩니다.
...
로그상으로는 정상 반영된 것처럼 보입니다. 이제 원격저장소에 반영해봅시다.

<br/><br/>

> ## 원격저장소에 반영하기

```git push```를 통해서 원격 저장소에 반영할 수 있습니다.
...
예상했듯이 rebase 등을 통해 커밋 히스토리가 합쳐진 경우는 강제 옵션과 함께 반영해야 합니다.

<pre class="line-numbers"><code class="language-bash" data-start="1">$ git push -f origin feature-rebase</code></pre>

강제 옵션(--force 또는 -f)을 붙여서 원격 저장소에 반영하면 앞서 생성했던 Pull Request에도 반영된 모습을 확인할 수 있습니다.
...
아래처럼 반영하고 싶은 커밋의 **바로 직전 커밋**의 해시 값을 이용하면 모두 선택 가능합니다.
...
다른 개발자와 같이 사용하는 브랜치에서 적용할 때는 특히 주의해야 합니다. 그러니까 다른 사람과 함께 만들고 있는 히스토리를
일방적으로 수정해서는 안됩니다.