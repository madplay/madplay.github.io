...
### public
`public`은 접근 지정자 또는 접근 제한자라고 부른다. 접근 지정자 중에서 `public`은 제한 없이 어디에서나 사용이 가능하다는
뜻이 된다. 반대로 상속 클래스 등에서만 사용할 수 있는 `protected`와 외부에서 접근 불가능한 `private` 등이 있다.
...
특히나 main 메서드처럼 프로그램의 시작점이 되는 요소는 객체를 생성하지 않아도 작업을 수행해야 하기 때문에 static이어야 한다.
