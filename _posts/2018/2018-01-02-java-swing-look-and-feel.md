...
# Java에서 Swing 이란?
스윙(Swing)은 자바 언어에서 GUI를 구현하기 위해 제공되는 라이브러리입니다.
자바에서 추구하는 WORE(Write Once, Run Everywhere)을 구현하기 위해 `JDK 1.2` 버전부터 사용되었습니다.

시스템 환경에 영향없이 어느 환경에서나 동일한 화면으로 표현될 수 있도록 시스템에 대한 의존도를 줄이고
컴포넌트들을 자바에서 직접 그려 구현하였습니다. 다만, 이렇게 동일하게 구현된 부분으로 인해 해당 시스템의
고유한 모습을 보여줄 수 없게 된 단점도 있습니다.

<br/><br/>

# LookAndFeel 이란?
환경에 상관없이 동일하게 구현된 모습으로 인해 시스템의 고유한 모습을 잃은 단점을 보완하기 위해 등장했습니다.
