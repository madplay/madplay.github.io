...
author:   madplay
tags: 	  nodejs 
description: Node.js를 Mac OS X에 설치하고 Hello World 예제 따라 해보기
category: Backend
comments: true
---
...
하지만 직접 설치해서 사용해보았을 때 여러 버전을 용이하게 테스트하는 데에는 **Node Version Manager(nvm)**를
...
설치된 이후에는 셸을 껐다가 다시 실행하거나 아래와 같이 ```source``` 명령어를 통해 적용합니다.
```bash
...
혹시나 ```nvm: command not found```와 같이 명령어를 찾을 수 없다면 ```~/.bash_profile``` 파일을 열어봅시다.
...
```macOS Mojave``` 버전 기준으로 ```.bashrc```에 환경 변수가 적용되더군요.
...
**Stable(또는 Current) 버전**의 경우는 홀수 번호대로 구성됩니다. 최신 기능을 제공하며 기존 기능의 개선에 초점이 맞춰진 버전으로
...
노드 설치도 끝났으므로 이제 간단한 예제를 진행해 봅시다. 혹시나 여러 버전의 노드를 설치했다면
...
언어를 배울 때의 가장 첫 단계라고 할 수 있는 **Hello World**를 웹 서버를 통해 출력해 봅시다.
...
위 예제의 결과는 인터넷 브라우저 주소창에 ```http://localhost:8000```을 입력하여 "Hello World"가 출력되는 것으로
