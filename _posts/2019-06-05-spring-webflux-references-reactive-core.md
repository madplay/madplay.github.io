...
기본 `ClientHttpConnector` 계약이 있다. 애플리케이션에서 사용되는 고수준(high-level)의 `WebClient`는 이 기본 계약을 기반으로 한다.
...
`Ordered` 인터페이스를 구현하여 우선순위를 표시할 수 있다.
...
사용된다. 이 부분은 **HTTP 메시지 코덱** 설정에 대한 섹션을 참조하라.
...
개발자가 설정할 수 있는 일부 옵션은 기본 코덱에 적용된다. 사용자 지정 코덱은 버퍼링 제한 또는 민감한 데이터 로깅과 같은 설정을 필요로 할 수 있다.
...
> ### 목차 가이드
> - <a href="/post/spring-webflux-references-dispatcherhandler" target="_blank">다음글 "1.3. DispatcherHandler" 로 이동</a>
