...
자바의 모든 클래스는 최상위 클래스인 Object 클래스의 메서드를 포함하고 있고 finalize 메서드는 그 메서드 중 하나입니다.
자바 가상 머신(Java Virtual Machine)이 리소스 누수(leak)를 방지하기 위해 실행하는 가비지 컬렉션이 수행될 때
더 이상 사용하지 않는 자원에 대한 정리 작업을 진행하기 위해 호출되는 종료자 메소드입니다.
<hr/>

> ### vs C++ 소멸자

자바는 메모리 관리를 JVM이 직접 진행합니다.
그렇기 때문에 개발자는 이러한 동적 할당의 해제에 대해서 크게 관여하지 않아도 됩니다.
...
위의 코드를 실행해보면 종료자를 멀리해야 하는 이유를 알 수 있는데요.
< Effective Java > 에서는 finalize 메소드 사용에 대해서 다음과 같이 언급하고 있습니다.


"_종료자는 사용하면 안 된다. 예측이 불가능하고 대체로 위험하고 일반적으로 필요하지 않다._"



자바의 finalize 메소드는 위에서 살펴본 것처럼 실행을 보장하지 않습니다.
이러한 특성을 가진 종료자 메소드에서 스트림을 닫는 행위를 하면 치명적일 수 있습니다.
...
가비지 컬렉션 실행을 요청하는 메소드가 있지만 반드시 실행을 보장하는 것은 아닙니다.
...
자바의 메모리 관련 자료를 찾아보면 객체를 사용하고 나서 쓸모가 없어진 경우 
null을 할당하는 방법을 추천합니다. 그렇게 하면 어떻게 될까요?

사용 유무에 따른 차이를 살펴보기 위해서 Garbage Collection 로그를 살펴봅시다.
...
resourceAllocate() 메소드가 종료되기 전까지 testArr1의 참조가 존재하기 때문이지요.
...
[http://blog.naver.com/madplay/220875720543](http://blog.naver.com/madplay/220875720543)


위의 링크된 포스팅 내용과 가비지 컬렉션 그리고 오늘 살펴본 finalize 메소드와의 관계를 요약해보면

가비지 컬렉션을 실행했지만, 메모리가 충분하다면 계속 참조하겠다. (Soft Reference)
가비지 컬렉션이 일어나기 전까지만 계속 참조하겠다. (Weak Reference)
finalize 메소드 호출 이후에도 참조하고 싶다. (Phantom Reference)
...
위에서 살펴본 finalize 메소드의 문제가 있음에도 이를 오버라이딩 하는 커스텀 클래스가 있고
이 클래스를 상속하고 finalize 메소드를 재정의하는 하위 클래스가 있다면 하위 클래스 객체가 소멸될 때
상위 클래스의 명시적 종료자를 반드시 호출하도록 강제하는 방어 기법을 사용하면 좋습니다.
...
익명 클래스를 통해서 Object의 finalize 메소드를 재정의합니다.
TestObject 객체가 가비지 컬렉션의 대상이 될 때, 멤버의 해제와  finalize 메소드가 호출됩니다.

위와 같은 방법이 아니라면 하위 클래스의 finalize 메소드에서 상위 클래스의 finalize 메소드를 호출하면 됩니다.
하지만 상위 클래스의 메소드 호출을 잊어버릴 가능성이 있지요.


결론적으로는 finalize 메소드는 실행을 보장하지 않습니다. 사용에 따른 장점이 적습니다.
