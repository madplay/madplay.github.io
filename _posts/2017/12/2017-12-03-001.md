...
더 이상 사용하지 않는 자원에 대한 정리 작업을 진행하기 위해 호출되는 종료자 메소드입니다.<br/>
<hr/>
...
위의 코드를 실행해보면 종료자를 멀리해야 하는 이유를 알 수 있는데요. < Effective Java > 서적에서는 finalize 메소드 사용에 대해서 다음과 같이 언급하고 있습니다.


<div class="caption"><center>"종료자는 사용하면 안 된다. 예측이 불가능하고 대체로 위험하고 일반적으로 필요하지 않다."</center></div>
...
finalize 메소드 호출 이후에도 참조하고 싶다. (Phantom Reference)<br/>
...
위에서 살펴본 finalize 메소드의 문제가 있음에도 이를 오버라이딩 하는 커스텀 클래스가 있고<br/>
이 클래스를 상속하고 finalize 메소드를 재정의하는 하위 클래스가 있다면 하위 클래스 객체가 소멸될 때<br/>
...
익명 클래스를 통해서 Object의 finalize 메소드를 재정의합니다.<br/>
TestObject 객체가 가비지 컬렉션의 대상이 될 때, 멤버의 해제와 finalize 메소드가 호출됩니다.

위와 같은 방법이 아니라면 하위 클래스의 finalize 메소드에서 상위 클래스의 finalize 메소드를 호출하면 됩니다.<br/>
하지만 상위 클래스의 메소드 호출을 잊어버릴 가능성이 있지요.


결론적으로는 finalize 메소드는 실행을 보장하지 않습니다. 사용에 따른 장점이 적습니다.<br/>
